...
class PlayState extends MusicBeatState
{
	...
	var cut:FlxSprite = new FlxSprite();
			...
			case 'chocostage':
				{
						...
						defaultCamZoom = 0.9;
						curStage = 'chocostage';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('chocostageback'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
					...
					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image(''));
						...
						bgc = new FlxSprite(0, 220);
						bgc.frames = Paths.getSparrowAtlas('Background');
						bgc.antialiasing = true;
						bgc.scrollFactor.set(1, 1);
						bgc.animation.addByPrefix('brrrrr', 'brrrrr', 24);
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'thorns')
					{
					 ...
					 else if (SONG.song.toLowerCase() == 'woman')
						{
							
							new FlxTimer().start(0, function(songTimer:FlxTimer)
								 {
									 camFollow.setPosition(dad.getMidpoint().x, boyfriend.getMidpoint().y);
									dad.alpha  = 0;
									
									FlxG.sound.play(Paths.sound('womanAppear', 'shared'));
									var epic:FlxSprite = new FlxSprite();
									epic.frames = Paths.getSparrowAtlas('sans');
									epic.animation.addByPrefix('idle', 'epick transformation', 24, false);
									epic.setGraphicSize(Std.int(epic.width * 1));
									epic.y = epic.y + 240;
									epic.x = epic.x + 50;
									
									add(epic);
									epic.animation.play('idle');
									new FlxTimer().start(4, function(deadTime:FlxTimer)
										{
											dad.alpha = 1;
											epic.alpha = 0;
											remove(dad);
											dad = new Character(100, 100, 'woman');
											add(dad);
											iconP2.animation.play('woman');
										});
										
									startCountdown();
								});
							}
					else if (SONG.song.toLowerCase() == 'chorus')
						{
							
									cut.frames = Paths.getSparrowAtlas('cutscenesimple');
									cut.animation.addByPrefix('idle', 'cutscene', 24, false);
									cut.setGraphicSize(Std.int(cut.width * 1));
									cut.y = cut.y + 400;
									cut.x = cut.x + 50;
									bgc.alpha = 0;
									add(cut);
							new FlxTimer().start(0, function(songTimer:FlxTimer)
								 {
									...
									gf.alpha =0;
									boyfriend.alpha = 0;
									FlxG.sound.play(Paths.sound('cutscene_music'));
									cutmove = false;
									cut.animation.play('idle');
									new FlxTimer().start(3, function(deadTime:FlxTimer)
										{
											FlxG.camera.zoom -= 0.4;
											cutmove= true;
											new FlxTimer().start(5.5, function(deadTime:FlxTimer)
												{
													cutmove = false;
													dad.alpha = 1;
													gf.alpha =1;
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curSong == 'Tacobelle')
			{
				
				//probably bad way to do this buttttttttt
				//im bad at this anyways so it doesnt matter
				if(curStep == 100)
					{
						dad.x = (dad.x - 200);
						dad.animation.play('sus');
